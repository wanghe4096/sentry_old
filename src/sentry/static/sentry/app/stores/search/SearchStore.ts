...
class SearchStore {
    ...
    initializeFieldsFromHash() {
        var parsedSearch = Immutable.Map<string, any>({});
        var parsedHash = Immutable.Map<string, any>({});
    ...
    _reloadSearchWithNewParam(param:string, value:any) {
        ...
        URLUtils.openLink(this.searchBaseLocation("") + "?" + Qs.stringify(searchURLParams.toJS()), false);
    }
