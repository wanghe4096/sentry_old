...
# urlpatterns += oauth_urls.urlpatterns
...
urlpatterns += patterns(
        ...
        # Settings - Projects
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/$',
            ProjectSettingsView.as_view(),
            name='sentry-manage-project'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/issue-tracking/$',
            ProjectIssueTrackingView.as_view(),
            name='sentry-project-issue-tracking'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/release-tracking/$',
            ProjectReleaseTrackingView.as_view(),
            name='sentry-project-release-tracking'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/keys/$',
            ProjectKeysView.as_view(),
            name='sentry-manage-project-keys'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/keys/new/$',
            CreateProjectKeyView.as_view(),
            name='sentry-new-project-key'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/keys/(?P<key_id>\d+)/edit/$',
            EditProjectKeyView.as_view(),
            name='sentry-edit-project-key'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/keys/(?P<key_id>\d+)/remove/$',
            RemoveProjectKeyView.as_view(),
            name='sentry-remove-project-key'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/keys/(?P<key_id>\d+)/disable/$',
            DisableProjectKeyView.as_view(),
            name='sentry-disable-project-key'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/keys/(?P<key_id>\d+)/enable/$',
            EnableProjectKeyView.as_view(),
            name='sentry-enable-project-key'),

        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/plugins/$',
            ProjectPluginsView.as_view(),
            name='sentry-manage-project-plugins'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/plugins/(?P<slug>[\w_-]+)/$',
            ProjectPluginConfigureView.as_view(),
            name='sentry-configure-project-plugin'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/plugins/(?P<slug>[\w_-]+)/reset/$',
            ProjectPluginResetView.as_view(),
            name='sentry-reset-project-plugin'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/plugins/(?P<slug>[\w_-]+)/disable/$',
            ProjectPluginDisableView.as_view(),
            name='sentry-disable-project-plugin'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/plugins/(?P<slug>[\w_-]+)/enable/$',
            ProjectPluginEnableView.as_view(),
            name='sentry-enable-project-plugin'),

        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/remove/$',
            RemoveProjectView.as_view(),
            name='sentry-remove-project'),

        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/tags/$',
            ProjectTagsView.as_view(),
            name='sentry-manage-project-tags'),

        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/quotas/$',
            ProjectQuotasView.as_view(),
            name='sentry-manage-project-quotas'),

        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/notifications/$',
            ProjectNotificationsView.as_view(),
            name='sentry-project-notifications'),

        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/rules/$',
            ProjectRulesView.as_view(),
            name='sentry-project-rules'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/rules/(?P<rule_id>\d+)/edit/$',
            ProjectRuleEditView.as_view(),
            name='sentry-edit-project-rule'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/rules/(?P<rule_id>\d+)/remove/$',
            ProjectRuleRemoveView.as_view(),
            name='sentry-remove-project-rule'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/settings/rules/new/$',
            ProjectRuleEditView.as_view(),
        ...
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_id>[\w_-]+)/issues/(?P<group_id>\d+)/$', react_page_view,
            name='sentry-group'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_id>[\w_-]+)/$', react_page_view,
            name='sentry-stream'),

        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/(?:group|issues)/(?P<group_id>\d+)/events/(?P<event_id>\d+)/replay/$', ReplayEventView.as_view(),
            name='sentry-replay'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/(?:group|issues)/(?P<group_id>\d+)/events/(?P<event_id_or_latest>(\d+|latest))/json/$', GroupEventJsonView.as_view(),
            name='sentry-group-event-json'),
        url(r'^(?P<organization_slug>[\w_-]+)/events/(?P<project_slug>[\w_-]+)/issues/(?P<group_id>\d+)/actions/(?P<slug>[\w_-]+)/', GroupPluginActionView.as_view(),
            name='sentry-group-plugin-action'),

        # url(r'', include('oauth.urls'), name='oauth'),
