...
const SavedSearchControls = React.createClass({
    ...
    _getNewSavedSearchButtons() {
        return <Button bsStyle="success" bsSize="small" onClick={this._openModal}>保存查询</Button>;
    },
    _getEditSavedSearchControls() {
        return (
            <DropdownButton bsSize="small" title="保存查询" id="saved-search-actions-dropdown">
                <MenuItem onSelect={this._openModal}>更新查询</MenuItem>
                <MenuItem divider/>
                <MenuItem onSelect={this._deleteSavedSearch}>删除查询</MenuItem>
            </DropdownButton>
        );
    },
    render() {
        return (
            <div style={{display: 'inline-block'}}>
                ...
                <BootstrapModalForm ref="saveSearchModal"
                                    title={this._isSearchSaved() ? "更新查询" : "保存查询"}
